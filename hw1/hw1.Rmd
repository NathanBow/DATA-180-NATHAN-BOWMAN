...
#COMPLETED TUTORIAL ON ALTERNATE DATA CAMP ACCOUNT NOT PART OF DATA 180 GROUP PRIOR TO RECEIVING INVITATION TO JOIN GROUP
...
There are 8 rows and 2 columns in 'my_df'. 
...
```{r}
my_df$Preference[my_df$Person == "Gisele"]
```
## Task 2
...
```{r}
my_func <- function(element) {if (element>5) {return("This element is greater than 5!")} else {return("This element is NOT greater than 5!")}}
```
## Task 3
...
```{r}
get_fibonacci <- function(n) {if (n== 1) {return(0)} else if (n == 2) {return(1)} else {return(get_fibonacci(n-1) + get_fibonacci(n-2))}}
```
b. Using your function in part (a), find and return the 19th value in the fibonacci sequence.
```{r}
get_fibonacci(19)
```
The 19th value in the fibonacci sequence is 2584.
